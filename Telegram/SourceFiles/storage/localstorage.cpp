...
void CustomLangPack::fetchCustomLangPack(const QString& langPackId, const QString& langPackBaseId) {
	...
	if (!langPackId.isEmpty() && !langPackBaseId.isEmpty() && !needFallback) {
		url.setUrl(qsl("https://ruagram.github.io/Localization/%1.json").arg(langPackId));
	} else {
		url.setUrl(qsl("https://ruagram.github.io/Localization/%1.json").arg(needFallback ? langPackBaseId : langPackId));
	}
...
void CustomLangPack::fetchFinished() {
	...
	if (statusCode == 404 && !langPackId.isEmpty() && !langPackBaseId.isEmpty() && !needFallback) {
		LOG(("Telegram Language pack not found! Fallback to main language: %1...").arg(langPackBaseId));
		needFallback = true;
...
void CustomLangPack::fetchError(QNetworkReply::NetworkError e) {
	...
	if (e == QNetworkReply::NetworkError::ContentNotFoundError) {
		...
		if (!langPackId.isEmpty() && !langPackBaseId.isEmpty() && !needFallback) {
			...
			LOG(("Telegram Language pack not found! Fallback to default language: English..."));
			loadDefaultLangFile();
			_chkReply = nullptr;
		}
	}
